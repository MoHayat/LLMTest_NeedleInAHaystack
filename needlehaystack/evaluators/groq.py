...
class GroqEvaluator(Evaluator):
    ...
    def __init__(
        ...
        """
        :param model_name: The name of the Groq model to use for evaluation.
        ...
        # Use the same API key as the model (NIAH_MODEL_API_KEY)
        api_key = os.getenv("NIAH_MODEL_API_KEY")
        if not api_key:
            raise ValueError(
                "NIAH_MODEL_API_KEY must be in env for using Groq evaluator."
            )

        self.api_key = api_key

        self.evaluator = ChatGroq(
            model=self.model_name, groq_api_key=self.api_key, **self.model_kwargs
        )
